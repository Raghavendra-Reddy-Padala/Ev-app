...
Widget _buildProfileBanner(ProfileController userController, BuildContext context) {
  return Container(
    padding: EdgeInsets.symmetric(vertical: 25.h, horizontal: 20.w),
    decoration: BoxDecoration(
      image: DecorationImage(
        image: NetworkImage(
          userController.userData.value?.data.banner?.isNotEmpty == true
            ...
            children: [
              Container(
                ...
                child: CircleAvatar(
                  ...
                  child: userController.userData.value?.data.avatar?.isNotEmpty == true
                      ? ClipRRect(
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      userController.userData.value?.data.firstName?.isNotEmpty == true
                    ...
                    SizedBox(height: 5.h),
                    Text(
                      userController.userData.value?.data.email?.isNotEmpty == true
...
Widget _buildProfileDetails(ProfileController userController, BuildContext context) {
  if (userController.isLoading.value) {
    ...
    if (userData.email.isNotEmpty) {
      displayFields.add({'key': 'Name', 'value': "${userData.firstName} "  "${userData.lastName}" , 'icon': Icons.account_circle});
    }
     if (userData.phone.isNotEmpty) {
      displayFields.add({'key': 'Phone', 'value': userData.phone, 'icon': Icons.phone});
    }
    if (userData.email.isNotEmpty) {
      displayFields.add({'key': 'Email', 'value': userData.email, 'icon': Icons.email});
    }
    ...
    if (userData.type.isNotEmpty) {
      displayFields.add({'key': 'Type', 'value': userData.type, 'icon': Icons.category});
    }
    ...
    if (userData.height > 0) {
      displayFields.add({'key': 'Height', 'value': '${userData.height} cm', 'icon': Icons.height});
    }
    if (userData.weight > 0) {
      displayFields.add({'key': 'Weight', 'value': '${userData.weight} kg', 'icon': Icons.monitor_weight});
    }
    ...
    if(userData.gender.isNotEmpty){
            displayFields.add({'key': 'Gender', 'value': userData.gender, 'icon': Icons.group});
    }
